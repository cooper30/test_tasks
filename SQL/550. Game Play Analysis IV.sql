...
*/
SELECT round(COUNT(t.player_id) / COUNT(a.player_id)::NUMERIC, 2) AS fraction
FROM (SELECT player_id,
             event_date,
             row_number() over(partition BY player_id ORDER BY event_date) AS rn
      FROM activity) a
         LEFT JOIN activity t ON t.player_id = a.player_id
    AND t.event_date = a.event_date + INTERVAL '1 days'
WHERE a.rn=1;